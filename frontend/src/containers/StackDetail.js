...
const StackDetail = (props) => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [getStack, location.state]);

  return stackInfo.length === 0 ? (
        ...
        <div className="row">
          <div className={`${style.colbg} px-3 col-md-7`}>
            <div className={`${style.cardblock} px-6`}>
              ...
              <p className="card-text">{stackInfo.description}</p>
              <a
                href={stackInfo.link ? stackInfo.link : '#'}
                className={`mt-4 ${style.btn2} mr-3`}
              >
                Read More
              </a>
              <Link
          ...
          <div className={`col-md-5 ${style.colpad}`}>
            <img className="d-block w-100" src={stackInfo.logo} alt="Stack logo" />
          </div>
        </div>
      </div>
    </div>
